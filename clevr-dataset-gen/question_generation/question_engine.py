...
with open(r'/content/sample_data/clevr-dataset-gen/output/CLEVR_scenes.json','r', encoding = 'utf-8') as file_open:
  ...
  def XXX(scene_struct, inputs, side_inputs):
    ...
    for idx in inputs[0]:
      ...
      if T==p1:
        ...
        if p2=="right" and (int(B[0])>300 and int(B[2])<300):
          output.append(scene_struct['objects'][idx]["size"]) 
        if int(B[2])>300:
          hint="this page is Centered layout"
      else:
        hint="There is no " +p1+" in the "+p2+" of page"
    if output ==[]:
      return hint
    else:
      return output
...
execute_handlers = {
  'make_position_title_l': xxx('title','left'),
  'make_position_title_r': xxx('title','right'),
  'make_position_text_l': xxx('text','left'),
  'make_position_text_r': xxx('text','right'),  
  'make_position_table_caption_l': xxx('table_caption','left'),
  'make_position_table_caption_r': xxx('table_caption','right'),
  'make_position_table_l': xxx('table','left'),
  'make_position_table_r': xxx('table','right'),
  'make_position_list_l': xxx('list','left'),
  'make_position_list_r': xxx('list','right'),  
  'make_position_figure_l': xxx('figure','left'),
  'make_position_figure_r': xxx('figure','right'),
  'make_position_figure_caption_l': xxx('figure_caption','left'),
  'make_position_figure_caption_r': xxx('figure_caption','right'),
